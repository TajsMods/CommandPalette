...
static func search(query: String, commands: Array[Dictionary], context: RefCounted, max_results: int = 12) -> Array[Dictionary]:
	...
	for cmd in commands:
		...
		if badge in ["OPT-IN", "GAMEPLAY"] and context and context.has_method("are_tools_enabled"):
			if not context.are_tools_enabled():
				continue
		var is_visible_func = cmd.get("is_visible", Callable())
		if is_visible_func.is_valid() and not is_visible_func.call(context):
			continue
		var enabled_func = cmd.get("is_enabled", Callable())
		if not enabled_func.is_valid():
			enabled_func = cmd.get("can_run", Callable())
		if enabled_func.is_valid() and not enabled_func.call(context):
			continue
