...
# Wire Drop scripts
const WireDropHandlerScript = preload("res://mods-unpacked/TajemnikTV-CommandPalette/extensions/scripts/wire_drop/wire_drop_handler.gd")
const WireDropConnectorScript = preload("res://mods-unpacked/TajemnikTV-CommandPalette/extensions/scripts/wire_drop/wire_drop_connector.gd")
const NodeCompatibilityFilterScript = preload("res://mods-unpacked/TajemnikTV-CommandPalette/extensions/scripts/wire_drop/node_compatibility_filter.gd")
...
func _log(message: String, is_error: bool = false) -> void:
    ...
    if is_error:
        CoreLog.log_error(LOG_NAME, message)
    else:
...
func initialize(tree: SceneTree, config, ui = null, mod_main_ref = null, registry_ref = null) -> void:
    if _initialized:
        return
...
func _init_node_metadata_service() -> void:
    ...
    CoreLog.log_info(LOG_NAME, "Node metadata service initialized (Inner Class)")
...
func _on_wire_dropped_on_canvas(origin_info: Dictionary, drop_position: Vector2) -> void:
    ...
    if compatible.is_empty():
        CoreServices.notify("exclamation", "No compatible nodes found")
        return
