...
const NodeDefinitionPanel = preload("res://mods-unpacked/TajemnikTV-CommandPalette/extensions/scripts/palette/node_definition_panel.gd")
const ResourceDefinitionPanel = preload("res://mods-unpacked/TajemnikTV-CommandPalette/extensions/scripts/palette/resource_definition_panel.gd")
const ModifierDefinitionPanel = preload("res://mods-unpacked/TajemnikTV-CommandPalette/extensions/scripts/palette/modifier_definition_panel.gd")
const PaletteTheme = preload("res://mods-unpacked/TajemnikTV-CommandPalette/extensions/scripts/palette/palette_theme.gd")

# Mode handlers
const CalculatorMode = preload("res://mods-unpacked/TajemnikTV-CommandPalette/extensions/scripts/palette/modes/calculator_mode.gd")
const HelpMode = preload("res://mods-unpacked/TajemnikTV-CommandPalette/extensions/scripts/palette/modes/help_mode.gd")
...
func show_node_definition(node_id: String) -> void:
    if not node_metadata_service:
        CoreServices.notify("exclamation", "Metadata service not available")
        return
    
    var details = node_metadata_service.get_node_details(node_id)
    if details.is_empty():
        CoreServices.notify("exclamation", "Could not load node details")
        return
    ...
    if controller and controller.has_method("get_node_metadata_service"):
        ...
        if service:
            ...
            if matching_nodes.is_empty():
                CoreServices.notify("exclamation", "No nodes found that add this modifier")
                return
...
func _show_nodes_with_port(shape: String, color: String, is_input: bool) -> void:
    ...
    if controller and controller.has_method("get_node_filter"):
        ...
        if filter:
            ...
            if nodes.is_empty():
                CoreServices.notify("exclamation", "No nodes found with this port type")
                return
        ...
        CoreServices.notify("exclamation", "Filter not available")
...
func hide_palette() -> void:
    ...
    if Engine.has_singleton("Sound"):
        CoreServices.play_sound("menu_close")
...
func _is_command_visible(cmd: Dictionary) -> bool:
    ...
    if registry and registry.has_method("is_visible"):
        if not registry.is_visible(cmd, context):
            return false
    else:
        if cmd.get("hidden", false):
            return false
        var is_visible_func = cmd.get("is_visible", Callable())
        if is_visible_func.is_valid() and context:
            if not is_visible_func.call(context):
                return false

    if registry and registry.has_method("is_enabled"):
        return registry.is_enabled(cmd, context)

    # Fallback to can_run if core registry is not available
...
func _execute_selected() -> void:
    ...
    var id = item.get("id", "")
    var executed := false
    if registry and registry.has_method("execute"):
        executed = registry.execute(id, context)
    elif registry and registry.has_method("run"):
        executed = registry.run(id, context)
    if executed:
...
func _execute_calculator_action(item: Dictionary) -> void:
    ...
    # Copy to clipboard
    CoreServices.clipboard_set(_calc_result)
    
    # Show notification
    CoreServices.notify("check", "Copied: " + _calc_result)
...
func _execute_node_def_selection(item: Dictionary) -> void:
    ...
    if item.get("id", "") == "_node_def_error":
        CoreServices.notify("exclamation", "Restart Required")
        CoreServices.play_sound("error")
        return
...
func _toggle_favorite_selected() -> void:
    ...
    if palette_config:
        ...
        CoreServices.notify("check" if is_now_fav else "exclamation",
