...
const LOG_NAME = "WireDropHandler"
...
var _config # TajsCoreCommandPalette reference
...
func _connect_signals() -> void:
	...
	_connected = true
	CoreLog.log_info(LOG_NAME, "Wire drop handler connected to signals")
...
func _on_connection_dropped(connection_id: String, connection_type: int) -> void:
	...
	if not is_instance_valid(origin_resource):
		CoreLog.log_warn(LOG_NAME, "Origin resource no longer valid, canceling wire drop")
		return
