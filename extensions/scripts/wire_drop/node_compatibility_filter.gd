...
func build_cache() -> void:
	...
	_cache_built = true
	CoreLog.log_info(LOG_NAME, "Built node compatibility cache for %d windows" % _window_connectors_cache.size())
...
func _extract_connector_data(rc: ResourceContainer) -> Dictionary:
	...
	if "Neuron" in rc.name:
		CoreLog.log_info(LOG_NAME, "DEBUG CompatibleFilter: Extracting from %s" % rc.name)
		CoreLog.log_info(LOG_NAME, "  - override_connector: %s" % str(rc.override_connector))
		CoreLog.log_info(LOG_NAME, "  - default_resource: %s" % str(rc.default_resource))
