...
const LOG_NAME = "WireDropConnector"
...
func spawn_and_connect(window_id: String, position: Vector2, origin_info: Dictionary) -> bool:
    ...
    if limit != -1 and Globals.max_window_count >= limit:
        CoreServices.notify("exclamation", "build_limit_reached")
        CoreServices.play_sound("error")
        spawn_failed.emit("Build limit reached")
        return false
    
    if not Utils.can_add_window(window_id):
        CoreServices.notify("exclamation", "Cannot add more of this window type")
        CoreServices.play_sound("error")
